def weighted_edit_distance(word1, word2, scores):
    ...
    for i in range(1, n + 1):  ## word2
        for j in range(1, m + 1): ## word1
            delect_cost = ed_delect_cost(j-1, i-1, word1, word2, scores)  ## delect a[i]   delect a[j]
...
def ed_replace_cost(i, j, word1, word2, scores):
    ...
    return max(1 - scores[i][c2]/scores[i][c1]*5, 0)

def char2num(char):
    special_char = ' !"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~'
    if char in '0123456789':
        num = ord(char) - ord('0') + 1
    elif char in 'abcdefghijklmnopqrstuvwxyz':#'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ':
        num = ord(char.lower()) - ord('a') + 11
    elif char in special_char:
        num = special_char.index(char)
    else:
